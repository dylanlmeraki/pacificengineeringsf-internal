// Frontend helper for calculating lead scores
...
export function calculateLeadScore({ prospect, interactions = [], outreach = [] }) {
  ...
  // Calculate engagement based on interactions (last 30 days)
  ...
  const probability = statusProbabilities[prospect?.status] || 10;

  // Segment
  let segment = "Cold Lead";
  if (engagement_score >= 70 || prospect_score >= 75) segment = "Hot Lead";
  else if (engagement_score >= 40 || prospect_score >= 50) segment = "Warm Lead";
  if (prospect?.deal_value >= 50000) segment = "High Value";
